...
#define LL_SYSTICK_CLKSOURCE_HCLK_DIV8 \
    0x00000000U /*!< AHB clock divided by 8 selected as SysTick
                                                                            clock source */
...
__STATIC_INLINE uint32_t LL_SYSTICK_IsActiveCounterFlag(void) {
    return (
        ((SysTick->CTRL & SysTick_CTRL_COUNTFLAG_Msk) == (SysTick_CTRL_COUNTFLAG_Msk)) ? 1UL :
                                                                                         0UL);
}
...
__STATIC_INLINE void LL_MPU_ConfigAttributes(uint32_t AttIndex, uint32_t Attributes) {
    ...
    /* When selected index is in range [4;7] */
        ...
        MODIFY_REG(
            MPU->MAIR1, (0xFFU << ((AttIndex - 4U) * 8U)), (Attributes << ((AttIndex - 4U) * 8U)));
    }
}
