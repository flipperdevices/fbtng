...
__STATIC_INLINE uint32_t LL_ICACHE_IsEnabledRegion(uint32_t Region) {
    return (
        (READ_BIT(*((__IO uint32_t*)(&(ICACHE->CRR0) + (1U * Region))), ICACHE_CRRx_REN) ==
         (ICACHE_CRRx_REN)) ?
            1UL :
            0UL);
}
...
__STATIC_INLINE void LL_ICACHE_SetRegionBaseAddress(uint32_t Region, uint32_t Address) {
    MODIFY_REG(
        *((__IO uint32_t*)(&(ICACHE->CRR0) + (1U * Region))),
        ...
        (((Address & 0x1FFFFFFFU) >> 21U) & ICACHE_CRRx_BASEADDR));
}
...
__STATIC_INLINE uint32_t LL_ICACHE_GetRegionBaseAddress(uint32_t Region) {
    return (READ_BIT(*((__IO uint32_t*)(&(ICACHE->CRR0) + (1U * Region))), ICACHE_CRRx_BASEADDR));
}
...
__STATIC_INLINE uint32_t LL_ICACHE_GetRegionRemapAddress(uint32_t Region) {
    return (
        (READ_BIT(*((__IO uint32_t*)(&(ICACHE->CRR0) + (1U * Region))), ICACHE_CRRx_REMAPADDR) >>
...
__STATIC_INLINE uint32_t LL_ICACHE_GetRegionSize(uint32_t Region) {
    return (
        READ_BIT(*((__IO uint32_t*)(&(ICACHE->CRR0) + (1U * Region))), ICACHE_CRRx_RSIZE) >>
