...
/**
  ...
  * @retval None
  */
__STATIC_INLINE void LL_USART_ConfigFIFOsThreshold(
...
__STATIC_INLINE uint32_t LL_USART_IsEnabledIT_RXNE_RXFNE(const USART_TypeDef* USARTx) {
    return (
        (READ_BIT(USARTx->CR1, USART_CR1_RXNEIE_RXFNEIE) == (USART_CR1_RXNEIE_RXFNEIE)) ? 1UL :
                                                                                          0UL);
}
...
__STATIC_INLINE uint32_t LL_USART_IsEnabled_AutonomousSendIdleFrame(const USART_TypeDef* USARTx) {
    return (
        (READ_BIT(USARTx->AUTOCR, USART_AUTOCR_IDLEDIS) == (USART_AUTOCR_IDLEDIS)) ? 0UL : 1UL);
}
...
__STATIC_INLINE uint32_t LL_USART_GetSelectedTrigger(const USART_TypeDef* USARTx) {
    return (
        uint32_t)((READ_BIT(USARTx->AUTOCR, USART_AUTOCR_TRIGSEL) >> USART_AUTOCR_TRIGSEL_Pos));
}
