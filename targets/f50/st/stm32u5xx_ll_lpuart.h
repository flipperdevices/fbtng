...
#define LL_LPUART_TRIG_POLARITY_FALLING \
    USART_AUTOCR_TRIGPOL /*!< LPUART triggered on falling edge */
/**
  * @}
  */
/**
...
/**
  ...
  * @retval None
  */
__STATIC_INLINE void LL_LPUART_ConfigFIFOsThreshold(
...
__STATIC_INLINE uint32_t LL_LPUART_IsEnabledIT_RXNE_RXFNE(const USART_TypeDef* LPUARTx) {
    return (
        (READ_BIT(LPUARTx->CR1, USART_CR1_RXNEIE_RXFNEIE) == (USART_CR1_RXNEIE_RXFNEIE)) ? 1UL :
                                                                                           0UL);
}
...
__STATIC_INLINE uint32_t LL_LPUART_IsEnabledIT_TXE_TXFNF(const USART_TypeDef* LPUARTx) {
    return (
        (READ_BIT(LPUARTx->CR1, USART_CR1_TXEIE_TXFNFIE) == (USART_CR1_TXEIE_TXFNFIE)) ? 1UL :
                                                                                         0UL);
}
...
__STATIC_INLINE uint32_t LL_LPUART_IsEnabled_AutonomousSendIdleFrame(const USART_TypeDef* LPUARTx) {
    return (
        (READ_BIT(LPUARTx->AUTOCR, USART_AUTOCR_IDLEDIS) == (USART_AUTOCR_IDLEDIS)) ? 0UL : 1UL);
}
...
__STATIC_INLINE uint32_t LL_LPUART_GetSelectedTrigger(const USART_TypeDef* LPUARTx) {
    return (
        uint32_t)((READ_BIT(LPUARTx->AUTOCR, USART_AUTOCR_TRIGSEL) >> USART_AUTOCR_TRIGSEL_Pos));
}
