...
#include <stdbool.h>
#include <stddef.h>
#define USE_SD_TRANSCEIVER 0 // FIXME
#include "stm32u5xx.h"
...
typedef struct {
    uint32_t
        ClockEdge; /*!< Specifies the SDMMC_CCK clock transition on which Data and Command change.
...
#define SDMMC_ERROR_BLOCK_LEN_ERR \
    ((uint32_t)0x00000080U) /*!< Transferred block length is not allowed for the card or the number of transferred bytes does not match the block length   */
...
#define SDMMC_ERROR_LOCK_UNLOCK_FAILED \
    ((uint32_t)0x00000800U) /*!< Sequence or password error has been detected in unlock command or if there was an attempt to access a locked card    */
...
#define SDMMC_ERROR_ERASE_RESET \
    ((uint32_t)0x00400000U) /*!< Erase sequence was cleared before executing because an out of erase sequence command was received                        */
...
#define SDMMC_CMD_SDMMC_SEN_OP_COND \
    ((uint8_t)5U) /*!< Sends host capacity support information (HCS) and asks the accessed card to send its operating condition register (OCR) content in the response on the CMD line.*/
...
#define SDMMC_CMD_HS_SEND_EXT_CSD \
    ((uint8_t)8U) /*!< Sends SD Memory Card interface condition, which includes host supply voltage information  and asks the card whether card supports voltage.                      */
...
#define SDMMC_CMD_SET_BLOCKLEN \
    ((uint8_t)16U) /*!< Sets the block length (in bytes for SDSC) for all following block commands (read, write, lock). Default block length is fixed to 512 Bytes. Not effective        */
/*!< for SDHS and SDXC.                                                                       */
#define SDMMC_CMD_READ_SINGLE_BLOCK \
    ((uint8_t)17U) /*!< Reads single block of size selected by SET_BLOCKLEN in case of SDSC, and a block of fixed 512 bytes in case of SDHC and SDXC.                                    */
#define SDMMC_CMD_READ_MULT_BLOCK \
    ((uint8_t)18U) /*!< Continuously transfers data blocks from card to host until interrupted by  STOP_TRANSMISSION command.                                                            */
...
#define SDMMC_CMD_WRITE_SINGLE_BLOCK \
    ((uint8_t)24U) /*!< Writes single block of size selected by SET_BLOCKLEN in case of SDSC, and a block of fixed 512 bytes in case of SDHC and SDXC.                                   */
...
#define SDMMC_CMD_ERASE_GRP_START \
    ((uint8_t)35U) /*!< Sets the address of the first write block to be erased. Reserved for each command system set by switch function command (CMD6).                                  */
#define SDMMC_CMD_ERASE_GRP_END \
    ((uint8_t)36U) /*!< Sets the address of the last write block of the continuous range to be erased. Reserved for each command system set by switch function command (CMD6).           */
...
#define SDMMC_CMD_LOCK_UNLOCK \
    ((uint8_t)42U) /*!< Sets/resets the password or lock/unlock the card. The size of the data block is set by the SET_BLOCK_LEN command.                                                */
#define SDMMC_CMD_APP_CMD \
    ((uint8_t)55U) /*!< Indicates to the card that the next command is an application specific command rather than a standard command.                                                   */
#define SDMMC_CMD_GEN_CMD \
    ((uint8_t)56U) /*!< Used either to transfer a data block to the card or to get a data block from the card for general purpose/application specific commands.                         */
...
#define SDMMC_CMD_APP_SD_SET_BUSWIDTH \
    ((uint8_t)6U) /*!< (ACMD6) Defines the data bus width to be used for data transfer. The allowed data bus widths are given in SCR register.                                                   */
#define SDMMC_CMD_SD_APP_STATUS \
    ((uint8_t)13U) /*!< (ACMD13) Sends the SD status.                                                            */
#define SDMMC_CMD_SD_APP_SEND_NUM_WRITE_BLOCKS \
    ((uint8_t)22U) /*!< (ACMD22) Sends the number of the written (without errors) write blocks. Responds with 32bit+CRC data block.                                                               */
#define SDMMC_CMD_SD_APP_OP_COND \
    ((uint8_t)41U) /*!< (ACMD41) Sends host capacity support information (HCS) and asks the accessed card to send its operating condition register (OCR) content in the response on the CMD line. */
...
#define SDMMC_STATIC_FLAGS                                                                        \
    ((uint32_t)(SDMMC_FLAG_CCRCFAIL | SDMMC_FLAG_DCRCFAIL | SDMMC_FLAG_CTIMEOUT |                 \
                ...
                SDMMC_FLAG_CMDREND | SDMMC_FLAG_CMDSENT | SDMMC_FLAG_DATAEND | SDMMC_FLAG_DHOLD | \
...
#define SDMMC_STATIC_DATA_FLAGS                                                                   \
    ((uint32_t)(SDMMC_FLAG_DCRCFAIL | SDMMC_FLAG_DTIMEOUT | SDMMC_FLAG_TXUNDERR |                 \
                SDMMC_FLAG_RXOVERR | SDMMC_FLAG_DATAEND | SDMMC_FLAG_DHOLD | SDMMC_FLAG_DBCKEND | \
                SDMMC_FLAG_DABORT | SDMMC_FLAG_IDMATE | SDMMC_FLAG_IDMABTC))
/**
  * @}
  */
...
/** @addtogroup HAL_SDMMC_LL_Group1
  * @{
  */
bool SDMMC_Init(SDMMC_TypeDef* SDMMCx, SDMMC_InitTypeDef Init);
/**
  * @}
  */
...
bool SDMMC_WriteFIFO(SDMMC_TypeDef* SDMMCx, uint32_t* pWriteData);
/**
  * @}
  */
...
/** @addtogroup HAL_SDMMC_LL_Group3
  * @{
  */
bool SDMMC_PowerState_ON(SDMMC_TypeDef* SDMMCx);
bool SDMMC_PowerState_Cycle(SDMMC_TypeDef* SDMMCx);
bool SDMMC_PowerState_OFF(SDMMC_TypeDef* SDMMCx);
uint32_t SDMMC_GetPowerState(const SDMMC_TypeDef* SDMMCx);

/* Command path state machine (CPSM) management functions */
bool SDMMC_SendCommand(SDMMC_TypeDef* SDMMCx, SDMMC_CmdInitTypeDef* Command);
uint8_t SDMMC_GetCommandResponse(const SDMMC_TypeDef* SDMMCx);
uint32_t SDMMC_GetResponse(const SDMMC_TypeDef* SDMMCx, uint32_t Response);

/* Data path state machine (DPSM) management functions */
bool SDMMC_ConfigData(SDMMC_TypeDef* SDMMCx, SDMMC_DataInitTypeDef* Data);
uint32_t SDMMC_GetDataCounter(const SDMMC_TypeDef* SDMMCx);
uint32_t SDMMC_GetFIFOCount(const SDMMC_TypeDef* SDMMCx);

/* SDMMC Cards mode management functions */
bool SDMMC_SetSDMMCReadWaitMode(SDMMC_TypeDef* SDMMCx, uint32_t SDMMC_ReadWaitMode);
/**
  * @}
  */
