...
#define IS_LL_DMA_ALL_CHANNEL_INSTANCE(INSTANCE, Channel)                      \
    ((((INSTANCE) == GPDMA1) &&                                                \
      (((Channel) == LL_DMA_CHANNEL_0) || ((Channel) == LL_DMA_CHANNEL_1) ||   \
       ...
       ((Channel) == LL_DMA_CHANNEL_8) || ((Channel) == LL_DMA_CHANNEL_9) ||   \
       ((Channel) == LL_DMA_CHANNEL_10) || ((Channel) == LL_DMA_CHANNEL_11) || \
       ((Channel) == LL_DMA_CHANNEL_12) || ((Channel) == LL_DMA_CHANNEL_13) || \
...
#define IS_LL_DMA_DATA_ALIGNMENT(__VALUE__)         \
    (((__VALUE__) == LL_DMA_DATA_ALIGN_ZEROPADD) || \
     ((__VALUE__) == LL_DMA_DATA_ALIGN_SIGNEXTPADD) || ((__VALUE__) == LL_DMA_DATA_PACK_UNPACK))
   ...
   */
    ...
    if(IS_LL_GPDMA_CHANNEL_INSTANCE(DMAx, Channel) != 0U) {
        LL_DMA_ConfigBurstLength(
            DMAx, Channel, DMA_InitStruct->SrcBurstLength, DMA_InitStruct->DestBurstLength);
    }
...
uint32_t LL_DMA_CreateLinkNode(
    ...
    if(DMA_InitNodeStruct->NodeType == LL_DMA_GPDMA_2D_NODE) {
        ...
        assert_param(
            IS_LL_DMA_BLKRPT_SRC_ADDR_UPDATE(DMA_InitNodeStruct->BlkRptSrcAddrUpdateMode));
        assert_param(
            IS_LL_DMA_BLKRPT_DEST_ADDR_UPDATE(DMA_InitNodeStruct->BlkRptDestAddrUpdateMode));
    ...
    */
        pNode->LinkRegisters[reg_counter] =
            ((DMA_InitNodeStruct->UpdateRegisters & (DMA_CLLR_UT1 | DMA_CLLR_UT2 | DMA_CLLR_UB1 |
