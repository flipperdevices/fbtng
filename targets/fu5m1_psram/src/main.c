...
int main(void) {
    ...
    //     const size_t size = SRAM3_SIZE / sizeof(uint32_t);

    //     {
    //         uint32_t start = DWT->CYCCNT;
    //         uint32_t* data = furi_hal_mem_pool_get_addr(FuriHalMemPoolPSram);
    ...
    //         float seconds =
    //             (float)(end - start) / furi_hal_cortex_instructions_per_microsecond() / 1000000;
    //         float speed_kb = (float)(count * size) / seconds / 1024 * 4;
    //         DBG_LOG_I(TAG, "PSRAM write took %f s, speed %f kb/s", seconds, speed_kb);
    //     }

    //     {
    //         uint32_t start = DWT->CYCCNT;
    //         uint32_t* data = furi_hal_mem_pool_get_addr(FuriHalMemPoolSram3);
    //         for(uint32_t z = 0; z < count; z++) {
    //             for(uint32_t i = 0; i < size; i++) {
    //                 data[i] = i;
    //             }
    ...
    //             (float)(end - start) / furi_hal_cortex_instructions_per_microsecond() / 1000000;
    //         float speed_kb = (float)(count * size) / seconds / 1024 * 4;
    //         DBG_LOG_I(TAG, "SRAM3 write took %f s, speed %f kb/s", seconds, speed_kb);
    //     }
    // }
