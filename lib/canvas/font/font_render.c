...
int8_t
    font_get_glyph(U8G2FontRender* font, U8G2FontGlyph* glyph, uint16_t search_position, char chr);
...
void font_render_glyph(U8G2FontRender* font, U8G2FontGlyph* glyph, int32_t x, int32_t y);
...
void u8g2_font_render_print(U8G2FontRender* font, int32_t x, int32_t y, const char* str) {
    while(*str) {
        const char* chr = str++;
        // if(*chr < 0x100) {
        u8g2_font_render_print_char(font, &x, y, *chr);
        // }
    }
}
...
size_t u8g2_font_str_width_get(U8G2FontRender* font, const char* str) {
    ...
    if(glyph.width != 0) {
        width -= glyph.pitch;
        width += glyph.width + glyph.x_offset;
    }
